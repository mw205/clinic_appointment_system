...
def clean_db():
    ...
    # Consultations use PROTECT to DoctorProfile, so remove consultation graph first.
...
def seed_data():
    ...
    # 2. Create Doctors
    print("Creating Doctors & Schedules...")
    ...
    # Ensure completed appointments always have one consultation,
    # and no appointment gets more than one consultation.
    for appointment in created_appointments:
        ...
        consultation, _ = ConsultationRecord.objects.get_or_create(
            ...
            defaults={
                ...
                "diagnosis": "Generated diagnosis for completed appointment",
                "notes": "Generated by seed_data.py",
                "completed_at": appointment.end_time,
            },
        )

        if consultation.completed_at is None:
            consultation.completed_at = appointment.end_time
            consultation.save(update_fields=["completed_at", "updated_at"])

        if not consultation.prescription_items.exists():
        ...
        if not consultation.requested_tests.exists():
